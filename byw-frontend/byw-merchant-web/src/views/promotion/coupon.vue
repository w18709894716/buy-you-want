  ...
  <div class="page-container">
    <el-card shadow="never">
      ...
      <el-table :data="tableData" v-loading="loading" stripe border>
        ...
        <el-table-column prop="usedCount" label="已领取" width="90" />
    ...
    >
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="优惠券名称" prop="name">
          <el-input v-model="form.name" placeholder="如：店铺满100减20" />
        </el-form-item>
...
const fetchData = async () => {
  loading.value = true
  try {
    const data: any = await request.get('/merchant/coupon/list', {
...
const handleDelete = async (row: any) => {
  ...
  try {
    await request.delete(`/merchant/coupon/${row.id}`)
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (dialogType.value === 'add') {
        await request.post('/merchant/coupon', payload)
        ElMessage.success('创建成功')
      } else {
        await request.put(`/merchant/coupon/${editingId.value}`, payload)
