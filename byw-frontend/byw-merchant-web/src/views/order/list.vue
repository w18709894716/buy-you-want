...
const searchForm = reactive({ orderNo: '', status: [] as number[] })
...
const fetchData = async () => {
  ...
  try {
    ...
    const data: any = await request.get('/merchant/order/list', { params })
...
const handleShip = async (row: any) => {
  ...
  try {
    // 拉取最新订单详情，获取各商品发货状态
    const data: any = await request.get(`/merchant/order/${row.orderNo}`)
...
const submitShip = async () => {
  ...
  await shipFormRef.value.validate(async (valid) => {
    ...
    try {
      await request.post(
        `/merchant/order/${shippingOrder.value.orderNo}/ship-items`,
        selectedItemIds.value,
