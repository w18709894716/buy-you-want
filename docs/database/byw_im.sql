...
CREATE TABLE t_conversation (
    ...
    entry VARCHAR(20) COMMENT '入口意图（买家点击客服按钮的页面：product-商品详情页 order-订单页 shop-店铺首页；NULL-未记录）',
...
CREATE TABLE t_im_dispatch_rule (
  ...
  intents VARCHAR(100) NULL COMMENT '匹配条件-入口意图（逗号分隔：product-商品详情页 order-订单页 shop-店铺首页 default-普通咨询；NULL=不按意图）',
